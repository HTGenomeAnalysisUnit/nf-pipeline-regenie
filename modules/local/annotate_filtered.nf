process ANNOTATE_FILTERED {
  ...
  awk '{OFS="\t"};{\$4=\$3"_%SEP%_"\$4; \$2=\$2-${params.annotation_interval_kb * 1000}; \$3=\$3+${params.annotation_interval_kb * 1000}}; {print \$0}' ${regenie_merged.baseName}.annotated.bed \
    | bedtools intersect -a stdin -b $genes -loj -header \
  ...
  bedtools merge -i ${regenie_merged.baseName}.closegenes.bed -c \$(echo {1..20} | tr " " ","),24 -o \$(printf 'distinct,%.0s' {1..21}) > ${regenie_merged.baseName}.final.bed
  ...
  (cat ${regenie_merged.baseName}.final.header.bed | head -n 1 && cat ${regenie_merged.baseName}.final.header.bed | tail -n +2 | sort -T work -k12 --general-numeric-sort --reverse) | gzip > ${regenie_merged.baseName}.annotated.txt.gz
  """
}
