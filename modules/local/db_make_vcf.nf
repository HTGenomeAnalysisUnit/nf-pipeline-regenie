process DB_MAKE_VCF_MULTI {
    ...
    | awk -F"\\t" 'BEGIN {OFS="\\t"; print "#CHROM","POS","ID","REF","ALT","QUAL","FILTER","INFO","FORMAT", "1"}; {OFS="\\t"}; NR>1 {if (\$13 > 7.301) GT="1/1"; else if (\$13 > 5) GT="0/1"; else GT="0/0"}; NR > 1 && \$13 > ${params.pval_threshold} {print \$1,\$2,\$3,\$4,\$5,"100","PASS",".","GT:INFO:LOGP:TYPE:GMODEL:EFFECT:SE:N:AF", GT":"\$7":"\$13":${trait_type}:${genetic_model}:"\$10":"\$11":"\$8":"\$6}') \
...
process DB_MAKE_VCF_SINGLE {
    ...
    output:
        tuple file("gwas_db-${params.project}.bcf"), file("gwas_db-${params.project}.bcf.csi"), emit: pheno_db
    ...
    bcftools view -Ob -o gwas_db-${params.project}.bcf ${phenotype}.vcf.gz
