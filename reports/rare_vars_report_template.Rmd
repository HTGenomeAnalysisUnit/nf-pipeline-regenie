...
params:
  ...
  date: 2021-08-09
  version: v0.2
  regenie_merged: caucasians_bmi_age_sex_pcs_cov_chr22_p21001_i0.regenie.gz
...
dt_clean <- dt %>% 
  ...
  mutate(rsid = GENE_NAME)
```

```{r, fig.height=600, fig.width=600}
dt_top_per_gene <- dt_clean %>%
  group_by(GENE_ID) %>% slice_max(pvalue, n=1)

p1 <- fast_manhattan(
  dt_top_per_gene,log10p = F,
  build='hg19', speed = "f", 
  color1 = "grey", color2 = "orange", pointsize = 2,
  highlight = dt_top_per_gene[dt_top_per_gene$pvalue > 3,]$GENE_NAME,
