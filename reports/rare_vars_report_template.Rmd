...
params:
  ...
  date: 2021-08-09
  version: v0.4
  regenie_merged: /ssu/gassu/shared/test_new_gwas_pipe/BMI_test_PCs/results/rarevar/p21001_i0.rarevars.regenie.gz
  regenie_filename: p21001_i0.rarevars.regenie.gz
  phenotype: p21001_i0
  ...
  regenie_step1_log: /ssu/gassu/shared/test_new_gwas_pipe/BMI_test_PCs/logs/BMI_test_PCs.step1.log
  regenie_step2_log: /ssu/gassu/shared/test_new_gwas_pipe/BMI_test_PCs/logs/BMI_test_PCs.step2.log
  phenotype_log: /ssu/gassu/shared/test_new_gwas_pipe/BMI_test_PCs/logs/regenie_pheno_input_withPCs.pheno.validated.log
  covariate_log: /ssu/gassu/shared/test_new_gwas_pipe/BMI_test_PCs/logs/regenie_pheno_input_withPCs.cov.validated.log
  manhattan_annotation_enabled: TRUE
  tophits_min_value: 1.3
  significance_stat_test: "BONF_bygroup"
---
...
fwrite(dt[order(dt$CHROM, dt$GENPOS),],file = paste0(prefix, ".correctedP"), 
...
dt_clean <- dt %>% 
  mutate(
    tmp_col=gsub("([^.]+)\\.(.+)","\\1@\\2", dt$ALLELE1) 
  ) %>% 
  separate(tmp_col,c("MASK","AFBIN"),sep = '@',extra = "merge", remove = FALSE) %>%
  select(-tmp_col)
  
dt_clean$GENE <- mapply(function(x,y)gsub(paste0('.',x),"",y),dt_clean$ALLELE1,dt_clean$ID) 
dt_clean$rsid <- dt_clean$GENE
...
The first plot represents results using LOG10P values.
...
datatable(dt_top_per_gene %>% filter(pvalue >= params$tophits_min_value) %>%
          ...
          options=list(scrollX=T, scrollY=500, fixedHeader = TRUE))
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE, fig.width=7, fig.height=7, dpi=300}
