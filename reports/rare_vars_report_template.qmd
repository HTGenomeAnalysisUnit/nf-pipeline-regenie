...
```{python joint_burden_plot}
#| fig-width: 3
#| fig-height: 3
#| include: false

burden_dt = dt_clean[dt_clean['TEST'].str.contains('-BURDEN-')]

tests = burden_dt['TEST'].unique().tolist()
sig_value_threshold
# For each line generate a regional plot using the plot_mqq function
for t in tests:
  sig_value_threshold = float(
  ...
  mydt = burden_dt[(burden_dt['TEST'] == t)]
  annotated_ids = mydt[(burden_dt['LOG10P'] >= sig_value_threshold)]['UNIQUEID'].tolist()
  ...
  plot_label = Markdown(f"#### Burden joint test: {t}\n\n")
  figure, log = regenieTable.plot_mqq(
    mode="qqm", skip=0, scaled=True,
    build=genome_build,
    anno='GENE', anno_set=annotated_ids,
...
tests_dt = dt_clean[dt_clean['TEST'].str.contains('ACATV|ACATO|SKAT|SKATO|ACATO-FULL|SKATO-ACAT')]
